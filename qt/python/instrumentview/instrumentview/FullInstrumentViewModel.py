    ...
    def calculate_projection(self, projection_option: str, axis: list[int]):
        ...
        if projection_option == self._SIDE_BY_SIDE:
            projection = iv_side_by_side.SideBySide(
                self._workspace, self._detector_ids, self._sample_position, root_position, self._detector_positions, np.array(axis)
            )
        elif projection_option.startswith("Spherical"):
            projection = iv_spherical.SphericalProjection(self._sample_position, root_position, self._detector_positions, np.array(axis))
        elif projection_option.startswith("Cylindrical"):
            projection = iv_cylindrical.CylindricalProjection(
                self._sample_position, root_position, self._detector_positions, np.array(axis)
            )
        else:
